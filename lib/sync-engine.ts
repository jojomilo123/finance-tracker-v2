...
import { useTransactionStore, TransactionRecord, AccountRecord, BudgetRecord, GoalRecord, NetWorthAsset, NetWorthLiability, SubscriptionRecord } from "@/stores/use-transaction-store";
...
export async function initRealtimeSync() {
  ...
  const channel = supabase
    ...
    .on(
      "postgres_changes",
      { event: "*", schema: "public", table: "user_goals" },
      () => pullRemoteGoals()
    )
    .subscribe();
...
export async function pullAllRemoteData() {
  ...
  await Promise.all([pullRemoteAccounts(), pullRemoteBudgets(), pullRemoteGoals()]);
}
