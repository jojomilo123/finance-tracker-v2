      ...
      const realSpent = transactions
        .filter(
          (t) =>
            ...
            (t.categoryName === b.categoryName || t.categoryId === b.id)
        )
        .reduce((sum, t) => sum + t.amount, 0);
      return { ...b, spentAmount: realSpent };
    });
  }, [budgets, transactions]);

  const totalBudget = computedBudgets.reduce((acc, curr) => acc + curr.budgetAmount, 0);
  const totalSpent = computedBudgets.reduce((acc, curr) => acc + curr.spentAmount, 0);
  const remainingBudget = totalBudget - totalSpent;
  const overbudgetCount = computedBudgets.filter((b) => b.spentAmount > b.budgetAmount).length;
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-semibold">Kategori Pengeluaran ({computedBudgets.length})</h3>
